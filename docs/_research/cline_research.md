...
> **Agent:** `[CLINE]` | **Phase:** 4 | **Source:** `./cline/` (HEAD at clone time)
> **Researcher:** Antigravity | **Date:** 2026-04-27
...
|---|---|---|
| `tool` | File write/edit proposed | Approve / Reject / Edit |
...
For **every tool use** (file write, command, browser action), Cline:
...
- **Notification hooks** — `UserPromptSubmit` hook fires for external integrations
...
| Tool Name | Handler | Parameters | Description |
|---|---|---|---|
| `execute_command` | `ExecuteCommandToolHandler` | `command`, `timeout` | Run shell commands via `CommandExecutor` |
| `read_file` | `ReadFileToolHandler` | `path`, `start_line`, `end_line` | Read file contents with optional line range |
| `write_to_file` | `WriteToFileToolHandler` | `path`, `content` | Create new files |
| `replace_in_file` | (shared handler) | `path`, `diff` | Search-and-replace edits in existing files |
| `apply_patch` | `ApplyPatchHandler` | `patch` | Apply unified diff patches |
| `search_files` | `SearchFilesToolHandler` | `path`, `regex`, `file_pattern` | Regex search across files (uses ripgrep) |
| `list_files` | `ListFilesToolHandler` | `path`, `recursive` | List directory contents |
| `list_code_definition_names` | `ListCodeDefinitionNamesToolHandler` | `path` | Tree-sitter-based symbol listing |
| `browser_action` | `BrowserToolHandler` | `action`, `url`, `coordinate`, `text` | Browser automation (Puppeteer) |
| `ask_followup_question` | `AskFollowupQuestionToolHandler` | `question` | Ask the user a question |
| `attempt_completion` | `AttemptCompletionHandler` | `result`, `command` | Declare task complete |
| `use_mcp_tool` | `UseMcpToolHandler` | `server_name`, `tool_name`, `arguments` | Invoke MCP server tool |
| `access_mcp_resource` | `AccessMcpResourceHandler` | `server_name`, `uri` | Read MCP resource |
| `load_mcp_documentation` | `LoadMcpDocumentationHandler` | `server_name` | Fetch MCP server docs |
| `web_fetch` | `WebFetchToolHandler` | `url` | Fetch URL content |
| `web_search` | `WebSearchToolHandler` | `query` | Web search |
| `new_task` | `NewTaskHandler` | `task` | Spawn a new task |
| `use_subagents` | `SubagentToolHandler` | (various) | Sub-agent delegation |
| `use_skill` | `UseSkillToolHandler` | `skill_name` | Execute a skill |
| `summarize_task` | `SummarizeTaskHandler` | `context`, `task_progress` | Context compaction |
| `condense` | `CondenseHandler` | — | Manual context condensation |
| `plan_mode_respond` | `PlanModeRespondHandler` | `response` | Respond in plan mode |
| `act_mode_respond` | `ActModeRespondHandler` | `response` | Respond in act mode |
| `new_rule` | (handler) | `rule` | Create a `.clinerules` entry |
| `report_bug` | `ReportBugHandler` | (various) | Report a bug |
| `generate_explanation` | `GenerateExplanationToolHandler` | (various) | Generate code explanation |
| `focus_chain` | (handler) | `task_progress` | Update focus chain / TODO list |
...
ToolExecutor.execute(block)
  ...
  → Check plan mode restrictions (FILE_NEW, FILE_EDIT, APPLY_PATCH blocked)
...
Cline has two modes:
- **Plan Mode** — Read-only tools only. File modification tools (`write_to_file`, `replace_in_file`, `apply_patch`, `new_rule`) are blocked.
- **Act Mode** — All tools available.

The LLM can request switching from Plan to Act mode via `act_mode_respond`.
...
- **Auto-discovery** — Can discover running Chrome instances on the local network
...
- Port scanning for Chrome debug port (default 9222)
- WebSocket endpoint discovery via `/json/version` endpoint
- Cached WebSocket endpoints for reconnection (1-hour TTL)
...
MCP tools appear alongside native Cline tools. The `McpHub` generates unique short keys (6-char nanoid prefixed with `c`) for each server to keep tool names compact. Tools are invoked via `use_mcp_tool` which delegates to `McpHub.callTool()`.
...
- **Slash commands** — `/newrule`, `/compact`, `/smol`, `/reportbug`, custom `.cline/commands/`
...
Cline creates git-based checkpoints at task boundaries:
- Commits workspace state before task starts
- Creates checkpoints after tool executions
...
| `PostToolUse` | After tool execution | No (observe only) | Logging, validation |
...
The `focus_chain` tool allows the LLM to maintain a living task progress checklist, updated after each tool call. This provides the user with real-time visibility into what the agent thinks it needs to do.
...
| **Hooks** | 7 lifecycle events | 3 events (pre/post/notification) | None | None |
