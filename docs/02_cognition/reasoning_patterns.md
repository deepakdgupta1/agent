# Reasoning Patterns
> Module: 02_cognition | Status: Phase 2 | Last Agent: Claude Code Synthesis
...
[AIDER] and [BABYAGI] reasoning patterns are documented in `02_cognition/task_decomposition.md`, `02_cognition/planning_strategies.md`, and `02_cognition/model_routing.md`. This document focuses on Claude Code's structural pattern; later phases (AutoGPT in Phase 6) will add explicit self-critique loops.
...
> Phase 1 agents [AIDER] and [BABYAGI] do not contribute to this module — Aider's edit-format reasoning is in `code_modification.md`, BabyAGI's task-creation prompts are in `task_decomposition.md`. Phase 6 will introduce [AUTOGPT]'s self-critique pattern as an explicit contrast.
